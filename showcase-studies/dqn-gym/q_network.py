...
class QNetwork():
    def __init__(self, args, num_actions):
        ...
        for layer in range(num_dense_layers):
            ...
            if layer == 0:
                ...
                policy_input = tf.reshape(last_policy_layer,
                                          shape=[-1, input_size])
            ...
            last_layers = self.dense_relu(policy_input, target_input,
        ...
        last_layers = self.dense_linear(
            last_policy_layer, last_target_layer,
            [args.dense_layer_shapes[-1][-1], num_actions])
        ...
        gpu_options = tf.GPUOptions(
            per_process_gpu_memory_fraction=0.33333)  # avoid using all vram for GTX 970
    ...
    def conv_relu(self, policy_input, target_input, kernel_shape, stride,
        ...
        with tf.variable_scope(name):
            ...
            activation = tf.nn.relu(
                tf.nn.conv2d(policy_input, weights, stride, 'VALID') + biases)
    ...
    def inference(self, obs):
        ...
        return np.squeeze(self.sess.run(self.policy_q_layer,
    ...
    def build_loss(self, error_clip, num_actions, double_dqn):
        ...
        with tf.name_scope("loss"):
            ...
            predictions = tf.reduce_sum(
                tf.mul(self.policy_q_layer, self.actions), 1)
