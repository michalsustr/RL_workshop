...
class ParallelQNetwork():
    def __init__(self, args, num_actions):
        ...
        with tf.device('/cpu:0'):
            self.observation = tf.placeholder(tf.float32,
                                              shape=[None, args.screen_dims[0],
                                                     args.screen_dims[1],
                                                     args.history_length],
                                              name="observation")
        self.actions = tf.placeholder(tf.float32, shape=[None, num_actions],
                                      name="actions")  # one-hot matrix because tf.gather() doesn't support multidimensional indexing yet
        self.rewards = tf.placeholder(tf.float32, shape=[None], name="rewards")
        self.next_observation = tf.placeholder(tf.float32,
                                               shape=[None, args.screen_dims[0],
                                                      args.screen_dims[1],
                                                      args.history_length],
                                               name="next_observation")
        self.terminals = tf.placeholder(tf.float32, shape=[None],
                                        name="terminals")
        ...
        for layer in range(num_conv_layers):
            ...
            last_layers = self.conv_relu(cpu_input, gpu_input, target_input,
                                         args.conv_kernel_shapes[layer],
                                         args.conv_strides[layer], layer)
        ...
        for layer in range(num_dense_layers):
            ...
            if layer == 0:
                ...
                target_input = tf.reshape(last_target_layer,
                                          shape=[-1, input_size])
            else:
                cpu_input = last_cpu_layer
                gpu_input = last_gpu_layer
            ...
            last_layers = self.dense_relu(cpu_input, gpu_input, target_input,
                                          args.dense_layer_shapes[layer], layer)
            last_cpu_layer = last_layers[0]
            last_gpu_layer = last_layers[1]
            last_target_layer = last_layers[2]
        ...
        # initialize q_layer
        last_layers = self.dense_linear(last_cpu_layer, last_gpu_layer,
                                        last_target_layer,
                                        [args.dense_layer_shapes[-1][-1],
                                         num_actions])
        ...
        if (args.optimizer == 'rmsprop') and (args.gradient_clip <= 0):
            self.train_op = tf.train.RMSPropOptimizer(
                args.learning_rate, decay=args.rmsprop_decay, momentum=0.0,
                epsilon=args.rmsprop_epsilon).minimize(self.loss)
        elif (args.optimizer == 'graves_rmsprop') or (
                args.optimizer == 'rmsprop' and args.gradient_clip > 0):
            self.train_op = self.build_rmsprop_optimizer(args.learning_rate,
                                                         args.rmsprop_decay,
        ...
        if not args.watch:
            param_hists = [tf.histogram_summary(name, param) for name, param in
                           zip(self.param_names, self.policy_network_params)]
            self.param_summaries = tf.merge_summary(param_hists)

        # start tf session
        gpu_options = tf.GPUOptions(
            per_process_gpu_memory_fraction=0.6)  # avoid using all vram for GTX 970
    ...
    def conv_relu(self, cpu_input, gpu_input, target_input, kernel_shape,
        ...
        with tf.variable_scope(name):
            ...
            with tf.device('/cpu:0'):
                cpu_activation = tf.nn.relu(
                    tf.nn.conv2d(cpu_input, weights, stride, 'VALID') + biases)

            with tf.device('/gpu:0'):
                gpu_activation = tf.nn.relu(
                    tf.nn.conv2d(gpu_input, weights, stride, 'VALID') + biases)

                target_weights = tf.Variable(weights.initialized_value(),
                                             trainable=False, name=(
                    "target_" + name + "_weights"))
                target_biases = tf.Variable(biases.initialized_value(),
                                            trainable=False,
                                            name=("target_" + name + "_biases"))

                target_activation = tf.nn.relu(
                    tf.nn.conv2d(target_input, target_weights, stride,
                                 'VALID') + target_biases)
    ...
    def dense_relu(self, cpu_input, gpu_input, target_input, shape, layer_num):
        ...
        with tf.variable_scope(name):
            ...
            with tf.device('/cpu:0'):
                cpu_activation = tf.nn.relu(
                    tf.matmul(cpu_input, weights) + biases)

            with tf.device('/gpu:0'):
                gpu_activation = tf.nn.relu(
                    tf.matmul(gpu_input, weights) + biases)
                ...
                target_activation = tf.nn.relu(
                    tf.matmul(target_input, target_weights) + target_biases)
    ...
    def dense_linear(self, cpu_input, gpu_input, target_input, shape):
        ...
        with tf.variable_scope(name):
            ...
            with tf.device('/gpu:0'):
                ...
                target_activation = tf.matmul(target_input,
    ...
    def inference(self, obs):
        ...
        return self.sess.run(self.cpu_q_layer,
                             feed_dict={self.observation: obs})

    def gpu_inference(self, obs):

        return self.sess.run(self.gpu_q_layer,
                             feed_dict={self.observation: obs})
    ...
    def build_loss(self, error_clip, num_actions, double_dqn):
        ...
        with tf.name_scope("loss"):
            ...
            predictions = tf.reduce_sum(tf.mul(self.gpu_q_layer, self.actions),
                                        1)
            ...
            if double_dqn:  # Double Q-Learning:
                ...
                indices = tf.range(0, tf.size(max_actions) * num_actions,
                                   num_actions) + max_actions
                max_action_values = tf.gather(
                    tf.reshape(self.target_q_layer, shape=[-1]), indices)
            else:
                max_action_values = tf.reduce_max(self.target_q_layer, 1)

            targets = tf.stop_gradient(self.rewards + (
            ...
            if error_clip >= 0:
                ...
                errors = (0.5 * tf.square(quadratic_part)) + (
                error_clip * linear_part)
            else:
    ...
    def train(self, o1, a, r, o2, t):
        ...
        loss = self.sess.run([self.train_op, self.loss],
                             feed_dict={self.observation: o1, self.actions: a,
                                        self.rewards: r,
    ...
    def build_rmsprop_optimizer(self, learning_rate, rmsprop_decay,
        ...
        with tf.name_scope('rmsprop'):
            ...
            if version == 'rmsprop':
                ...
                avg_grads = [tf.Variable(tf.zeros(var.get_shape())) for var in
                             params]
                avg_square_grads = [tf.Variable(tf.zeros(var.get_shape())) for
                                    var in params]

                update_avg_grads = [grad_pair[0].assign(
                    (rmsprop_decay * grad_pair[0]) + (
                    (1 - rmsprop_decay) * grad_pair[1]))
                                    for grad_pair in zip(avg_grads, grads)]
                update_avg_square_grads = [grad_pair[0].assign(
                    (rmsprop_decay * grad_pair[0]) + (
                    (1 - rmsprop_decay) * tf.square(grad_pair[1])))
                                           for grad_pair in
                                           zip(avg_square_grads, grads)]
                avg_grad_updates = update_avg_grads + update_avg_square_grads

                rms = [tf.sqrt(avg_grad_pair[1] - tf.square(
                    avg_grad_pair[0]) + rmsprop_constant)
                       for avg_grad_pair in zip(avg_grads, avg_square_grads)]

                rms_updates = [grad_rms_pair[0] / grad_rms_pair[1] for
                               grad_rms_pair in zip(grads, rms)]
    ...
    def get_biases(self, shape, name):
        ...
        std = 1 / math.sqrt(fan_in)
        return tf.Variable(
            tf.random_uniform([shape[-1]], minval=(-std), maxval=std),
            name=(name + "_biases"))
