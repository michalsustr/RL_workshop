...
class ExperienceMemory:
    def __init__(self, args, num_actions):
        ...
        self.observations = np.empty(
            (self.capacity, self.screen_dims[0], self.screen_dims[1]),
            dtype=np.uint8)
    ...
    def get_state(self, indices):
        ...
        '''
        state = np.empty((
                         len(indices), self.screen_dims[0], self.screen_dims[1],
                         self.history_length))
        count = 0

        for index in indices:
            frame_slice = np.arange(index - self.history_length + 1,
                                    (index + 1))
            state[count] = np.transpose(
                np.take(self.observations, frame_slice, axis=0), [1, 2, 0])
            count += 1
        return state
    ...
    def get_batch(self):
        ...
        while len(samples) < self.batch_size:
            ...
            if self.size < self.capacity:  # make this better
                ...
                index = (self.current + random.randrange(self.history_length,
        ...
        a = np.eye(self.num_actions)[
            self.actions[samples]]  # convert actions to one-hot matrix
